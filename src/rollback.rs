...
fn check_rollback_window(
    ...
    if elapsed > window {
        println!("⚠️  Rollback window expired ({} ago)", format_duration(elapsed));
...
fn get_previous_value(secret_name: &str, env: &str) -> Result<String> {
    ...
    let rotate_entries: Vec<_> = entries
        .iter()
        .filter(|e| matches!(e.action, crate::audit::AuditAction::Rotate))
        .collect();
    
    if rotate_entries.len() < 2 {
        anyhow::bail!("No previous value found in audit logs");
    }
    
    anyhow::bail!("Previous value retrieval requires storing encrypted values in audit logs (not yet implemented)");
}
